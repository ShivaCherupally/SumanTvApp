...
public class FrescoFaceCenterCropNew extends BasePostprocessor {
    ...
    public FrescoFaceCenterCropNew(int width, int height) {
        this.width = width;
        this.height = height;
    }

    public FrescoFaceCenterCropNew(int width, int height, int unit) {
        if (unit == PIXEL) {
            this.width = width;
            this.height = height;
        } else if (unit == DP) {
    ...
    public CloseableReference<Bitmap> process(Bitmap sourceBitmap, PlatformBitmapFactory bitmapFactory) {
        ...
        if (scaleX != scaleY) {
            faceDetector = new
                    FaceDetector.Builder(AppController.getInstance().getCurrentRegisteredActivity())
