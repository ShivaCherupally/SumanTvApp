...
import info.sumantv.flow.imagepicker.helper.CameraHelper;
import info.sumantv.flow.imagepicker.helper.LogHelper;
import info.sumantv.flow.imagepicker.helper.PermissionHelper;
import info.sumantv.flow.imagepicker.listener.OnBackAction;
import info.sumantv.flow.imagepicker.listener.OnFolderClickListener;
...
import info.sumantv.flow.imagepicker.widget.ImagePickerToolbar;
import info.sumantv.flow.imagepicker.widget.ProgressWheel;
import info.sumantv.flow.imagepicker.widget.SnackBarView;

/**
 * Created by Uday on 7/31/16.
 */
...
public class ImagePickerActivity extends AppCompatActivity implements ImagePickerView {
    ...
    boolean isInSideFolderView = true;
    ...
    private View.OnClickListener multipleImageClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {

            ArrayList<Image> images_ = new ArrayList<>();
            ImagePicker.with(BottomMenuActivity.getInstance())
                    .setFolderMode(true)
                    .setCameraOnly(false)
                    .setFolderTitle("Album")
                    .setMultipleMode(true)
                    .setGif(false)
                    .setSelectedImages(images_)
                    .setMaxSize(10)
                    .setBackgroundColor("#212121")
                    .setAlwaysShowDoneButton(false)
                    .setMultipleImages(true)
                    .setRequestCode(Utility.generateRequestCodes().get("ADD_MEDIA_TO_STORE"))
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupToolbar();
        if (config.getFolder() != null && !config.getFolder().getFolderName().isEmpty()) {
            setImageAdapter(config.getFolder().getImages(), config.getFolder().getFolderName());
        }
    ...
    private void setupComponents() {
        ...
        presenter = new ImagePickerPresenter(new ImageFileLoader(this, config.isGifVisible(), config.gettMultipleImages()));
        presenter.attachView(this);
    }

    private void setupToolbar() {
        toolbar.config(config);
        toolbar.isInSideFolderView = isInSideFolderView;
        ...
        toolbar.setOnMultipleImageClickListener(multipleImageClickListener);
    ...
    private void captureImage() {
        ...
        getSupportFragmentManager().beginTransaction().replace(R.id.frameLayoutBottom, CameraOptionsFragment.newInstance(true, null), "CameraOptionsFragment").commit();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if (requestCode == Config.RC_CAPTURE_VIDEO && resultCode == RESULT_OK) {
            presenter.finishCaptureImage(this, data, config);
            int maxSize = config.getMaxSize();
            int reqCode = config.getRequestCode();
            boolean isFeed = config.isFeed();
            boolean isMul = config.isMultipleImages();
            boolean isGif = config.isGifVisible();
            ArrayList<Image> images_ = new ArrayList<>();
            Activity activity = this;
            if (BottomMenuActivity.getInstance() != null) {
                activity = BottomMenuActivity.getInstance();
            } else {
                activity = HelperActivity.getInstance();
            }
    ...
    public void openCameraSnap() {
        Fragment fragment = getSupportFragmentManager().findFragmentByTag("CameraOptionsFragment");
        if (fragment != null) {
    ...
    public void openCameraVideo() {
        ...
        presenter.captureVideo(this, config, Config.RC_CAPTURE_VIDEO);
    }
}
