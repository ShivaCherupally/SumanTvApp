package info.dkapp.flow.imagepicker.helper;
...
import info.dkapp.flow.imagepicker.model.SavePath;
...
public class ImageHelper {
    ...
    public static File createVideoFile(SavePath savePath) {
        ...
        String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String imageFileName = "DCM_" + timeStamp;
    ...
    public static boolean isGifFormat(Image image) {
        String extension = image.dataFilePath.substring(image.dataFilePath.lastIndexOf(".") + 1, image.dataFilePath.length());
        return extension.equalsIgnoreCase("gif");
    }
    public static boolean isVideoFormat(Image image) {
        if (image.mediaType == MediaType.IMAGE){
            return false;
        }
        return true;
    }
